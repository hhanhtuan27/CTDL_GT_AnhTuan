...
// Hàm duyệt cây theo phương pháp NLR và ghi ra tập tin nhị phân
void preOrderToFile(NODE* root, ofstream &file) {
    if (root == nullptr) return;
    
    file.write(reinterpret_cast<const char*>(&root->data), sizeof(root->data));
    preOrderToFile(root->left, file);
    preOrderToFile(root->right, file);
}
